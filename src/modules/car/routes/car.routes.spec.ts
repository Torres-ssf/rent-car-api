...
describe('Car Register Endpoint', () => {
  it('ensure that car name, brand, and dailyValue are present', async () => {
  ...
  it('ensure that car name and brand have at least 3 chars length', async () => {
    const res = await request(app).post('/car').send({
      name: 'as',
      brand: 'sa',
      daily_value: 900,
  ...
  it('ensure that car name and brand length are not higher than 80 chars', async () => {
    const res = await request(app).post('/car').send({
      name:
